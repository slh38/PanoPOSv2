...
public sealed class UrunFiyatController : ControllerBase
{
    private readonly PanoPosDbContext _db;
    public UrunFiyatController(PanoPosDbContext db) => _db = db;

    [HttpPut("{id:long}")]
    public async Task<ActionResult<UrunFiyatDto>> Guncelle(long id, UrunFiyatGuncelleRequestDto request, CancellationToken ct)
    {
        var fiyat = await _db.UrunFiyatlari.SingleOrDefaultAsync(x => x.Id == id, ct)
            ?? throw new UygulamaHatasi(404, "Fiyat bulunamadi", "Urun fiyati bulunamadi.", "product_price_not_found");
        if (fiyat.SilindiMi)
        ...
        var cakisma = await _db.UrunFiyatlari.AnyAsync(x => x.Id != fiyat.Id && x.TenantId == fiyat.TenantId && x.UrunSatisBirimiId == fiyat.UrunSatisBirimiId && x.FiyatTipiId == fiyat.FiyatTipiId && x.AktifMi, ct);
        ...
        return Ok(new UrunFiyatDto { Id = fiyat.Id, FiyatTipiId = fiyat.FiyatTipiId, Fiyat = fiyat.Fiyat, ParaBirimKodu = fiyat.ParaBirimKodu });
    }
}
