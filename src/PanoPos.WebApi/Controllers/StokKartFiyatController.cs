...
public sealed class StokKartFiyatController : ControllerBase
{
    ...
    {
        var fiyat = await _db.StokKartFiyatlari.TenantKapsami(_db).SingleOrDefaultAsync(x => x.Id == id, ct)
        ...
        var cakisma = await _db.StokKartFiyatlari.TenantKapsami(_db).AnyAsync(x => x.Id != fiyat.Id && x.TenantId == fiyat.TenantId && x.StokKartSatisBirimiId == fiyat.StokKartSatisBirimiId && x.FiyatTipiId == fiyat.FiyatTipiId && x.AktifMi, ct);
