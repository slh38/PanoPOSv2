...
public sealed class AdisyonServisi : IAdisyonServisi
{
    ...
    public async Task<AdisyonDto> AdisyonAcAsync(AdisyonAcRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        {
            request.AcanKullaniciId = IslemKapsami.Kimlik(request.AcanKullaniciId, context.KullaniciId);
            request.AcanCihazId = IslemKapsami.Kimlik(request.AcanCihazId, context.CihazId);
        }
        ...
        var masa = await _dbContext.Masalar.SubeKapsami(_dbContext).SingleOrDefaultAsync(x => x.Id == request.MasaId, cancellationToken)
        ...
        var kullaniciVar = await _dbContext.Kullanicilar.TenantKapsami(_dbContext).AnyAsync(x => x.Id == request.AcanKullaniciId && x.AktifMi, cancellationToken);
        if (!kullaniciVar)
        {
            throw new UygulamaHatasi(404, "Kullanici bulunamadi", "Kullanici bulunamadi.", "kullanici_not_found");
        }

        var cihazVar = await _dbContext.Cihazlar.SubeKapsami(_dbContext).AnyAsync(x => x.Id == request.AcanCihazId && x.AktifMi, cancellationToken);
        if (!cihazVar)
        {
            throw new UygulamaHatasi(404, "Cihaz bulunamadi", "Cihaz bulunamadi.", "cihaz_not_found");
        }

        var acikAdisyonVar = await _dbContext.Adisyonlar.SubeKapsami(_dbContext).AnyAsync(x => x.MasaId == request.MasaId && x.Durum == AdisyonDurumu.Acik, cancellationToken);
        if (acikAdisyonVar)
        {
    ...
    public async Task<AdisyonDto> AdisyonKapatAsync(AdisyonKapatRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        var adisyon = await _dbContext.Adisyonlar.SubeKapsami(_dbContext)
            .Include(x => x.Masa)
    ...
    public async Task<AdisyonDto?> AcikAdisyonGetirAsync(long masaId, CancellationToken cancellationToken = default)
    {
        ...
        var adisyonId = await _dbContext.Adisyonlar.SubeKapsami(_dbContext)
            .AsNoTracking()
    ...
    {
        return _dbContext.Adisyonlar.SubeKapsami(_dbContext)
            .AsNoTracking()
