...
public sealed class RenkServisi : IRenkServisi
{
    ...
    public async Task<RenkDto> OlusturAsync(RenkOlusturRequestDto request, CancellationToken cancellationToken = default)
    {
        ...
        if (await _dbContext.Renkler.TenantKapsami(_dbContext).AnyAsync(x => x.TenantId == sube.TenantId && x.Kod == kod, cancellationToken))
        {
    ...
    {
        return await _dbContext.Renkler.TenantKapsami(_dbContext).OrderBy(x => x.Ad).Select(x => new RenkDto { Id = x.Id, Ad = x.Ad, Kod = x.Kod }).ToListAsync(cancellationToken);
    }
}
