...
public sealed partial class FaturaServisi
{
    private async Task LockOrderAsync(long id, CancellationToken ct)
    {
        ...
        await _dbContext.Database.GetDbConnection().ExecuteScalarAsync<long?>(new CommandDefinition(
            "SELECT Id FROM Siparis WITH (UPDLOCK, HOLDLOCK) WHERE Id=@Id AND SilindiMi=0",
            new { Id = id }, _dbContext.Database.CurrentTransaction!.GetDbTransaction(), cancellationToken: ct));
    }

    private async Task<long> ResolveDepoAsync(Siparis order, long? requested, CancellationToken ct)
    {
        if (!await _dbContext.Subeler.AnyAsync(x => x.Id == order.SubeId && x.TenantId == order.TenantId && x.AktifMi, ct))
            throw StockError("Siparisin tenant/sube bilgisi gecersiz.");
        var query = _dbContext.Depolar.Where(x => x.TenantId == order.TenantId &&
    ...
    {
        if (await _dbContext.StokFisleri.IgnoreQueryFilters().AnyAsync(x => x.FaturaId == invoice.Id, ct))
        ...
        var lines = await _dbContext.FaturaDetaylari.Where(x => x.FaturaId == invoice.Id).OrderBy(x => x.Id).ToListAsync(ct);
        foreach (var line in lines)
        {
            ...
            if (!await _dbContext.StokKartler.AnyAsync(x => x.Id == line.StokKartId && x.TenantId == invoice.TenantId && x.AktifMi, ct) ||
                !await _dbContext.StokKartSatisBirimleri.AnyAsync(x => x.Id == line.StokKartSatisBirimiId &&
            ...
            if (line.StokKartVaryantId.HasValue && !await _dbContext.StokKartVaryantlari.AnyAsync(x =>
